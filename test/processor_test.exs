defmodule Cue.ProcessorTest do
  ...
  describe "processes jobs" do
    ...
    test "calls handler, sets fields on job when done", context do
      ...
      state = %{
        test: context.test,
        fun: make_counter(context.test, {context.test, :count})
      }
      ...
      assert get_count(context.test, {context.test, :count}) == 1
    end
    ...
    test "if max retries set stop after that many failures - crashes", context do
      ...
      counter = make_counter(context.test, {context.test, :count})
      ...
      assert get_count(context.test, {context.test, :count}) == 4
    end
    ...
    test "if max retries set stop after that many failures - errors", context do
      ...
      state = %{
        ...
        test: context.test,
        fun: make_counter(context.test, {context.test, :count}),
        error_fun: make_counter(context.test, {context.test, :error_count})
      }
      ...
      assert get_count(context.test, {context.test, :count}) == 4
      assert get_count(context.test, {context.test, :error_count}) == 3
    end
    ...
    test "one-off jobs not retried", context do
      ...
      assert get_count(context.test, {context.test, :count}) == 2
    end
  ...
  defp make_counter(agent, key) do
    fn ->
